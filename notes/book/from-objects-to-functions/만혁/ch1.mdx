...
class E02_DiscountTest {
    class Promotion(
        ...
        fun decode(): DiscountRule {
            val (x, y) = strategy.split("x").map { it.toInt() }
            return DiscountRule(x, y)
        }
    ...
    class CashRegister(
        val products: List<com.ubertob.fotf.exercises.chatper1.Product>,
        val promotions: List<com.ubertob.fotf.exercises.chatper1.Promotion>
    ) {

        fun checkout(items: List<String>): Double {
            val itemMap = items.groupingBy { it }.eachCount()

            return itemMap.entries.sumOf { (name, count) ->
                ...
                product
                    ?.let { calculateTotalPrice(it, count, rule) }
                    ?: 0.0
            }
        }

        fun calculateDiscount(count: Int, rule: DiscountRule): Int {
            val discounted = count / rule.buy
            val remaining = count % rule.buy

            return discounted * rule.pay + remaining
        }

        fun calculateTotalPrice(
            product: com.ubertob.fotf.exercises.chatper1.Product,
            count: Int,
            rule: DiscountRule
        ): Double {
            val discounted = calculateDiscount(count, rule)
